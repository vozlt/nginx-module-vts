...
=== TEST 13: literal percent-encoded zone is not decoded again
--- http_config
    vhost_traffic_status_zone;
--- config
    location /status {
    ...
    location /test_literal_percent {
        set $vol "test%7Cvalue";
        vhost_traffic_status_filter_by_set_key $vol storage::$server_name;
        return 200 "filter:OK";
    }
--- request eval
[
    'GET /test_literal_percent',
    'GET /status/control?cmd=delete&group=filter&zone=storage::localhost@test%7Cvalue',
]
--- response_body_like eval
[
...
=== TEST 14: escaped percent reaches a literal percent-encoded zone
--- http_config
    vhost_traffic_status_zone;
--- config
    location /status {
...
--- request eval
[
    'GET /test_escaped_percent',
    'GET /status/control?cmd=delete&group=filter&zone=storage::localhost@test%257Cvalue',
]
--- response_body_like eval
[
    'OK',
